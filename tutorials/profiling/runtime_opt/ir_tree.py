...
class IrTree(object):
    ...
    def __init__(self, graph=None):
        if graph is None:
            assert (DEBUG is True), 'reading from json disabled in non-debug mode'
        ...
        # parse the graph to construct the tree
    ...
    def convert2Tree(self, graph):
        ...
        for node_id, (node_vals, shape) in enumerate(zip(graph['nodes'], graph['shape'][1])):
            ...
            for parent_id in input_ids:
                parent_id = parent_id[0]  # some peculiarity in relay graph format, remaining items are always 0
    ...
    def bottom_up_edge_traversal(self, leaf_nodes=None, visited=None, traversal=None):
